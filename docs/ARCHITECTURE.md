...
AI Metadata Cleaner is a small Python CLI organized around file-type handlers.

## Entry Points

- `ai-metadata-cleaner` is exposed by Poetry through `m_c.cli.main:cli`.
...
- `ai-metadata-cleaner web` runs a local-only single-page Web UI that reuses the
