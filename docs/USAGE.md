...
```bash
ai-metadata-cleaner --verbose view my_photo.jpg
```

Write logs to a rotating file:

```bash
ai-metadata-cleaner --log-file ./ai-metadata-cleaner.log delete sample.jpg
```
...
```bash
ai-metadata-cleaner view my_photo.jpg
```

Print metadata in a stable JSON envelope:

```bash
ai-metadata-cleaner view my_photo.jpg --json
```

Write metadata JSON to a file:

```bash
ai-metadata-cleaner view my_photo.jpg --json-output reports/metadata.json
```

Remove metadata from one file:

```bash
ai-metadata-cleaner delete my_photo.jpg
```

Remove metadata and choose an output file:

```bash
ai-metadata-cleaner delete my_photo.jpg --output cleaned/my_photo.jpg
```

Process a directory recursively:

```bash
ai-metadata-cleaner delete ./images --output ./cleaned-images
```
...
```bash
ai-metadata-cleaner delete ./images --json-summary
```

Write the summary to a JSON file:

```bash
ai-metadata-cleaner delete ./images --summary-file reports/summary.json
```

You can also use the shared JSON output option:

```bash
ai-metadata-cleaner delete ./images --json-output reports/summary.json
```

Include SHA-256 checksums in JSON summaries:

```bash
ai-metadata-cleaner delete ./images --summary-file reports/summary.json --checksums
```

Choose a different checksum algorithm:

```bash
ai-metadata-cleaner delete ./images --json-summary --checksums --checksum-algorithm sha512
```

Control JSON report verbosity for large batches:

```bash
ai-metadata-cleaner delete ./images --json-summary --report-detail compact
ai-metadata-cleaner delete ./images --summary-file reports/summary.json --report-detail summary
```

List only failed per-file entries while preserving top-level counts:

```bash
ai-metadata-cleaner delete ./images --json-summary --report-filter failed
```

Preserve source access and modification times on cleaned files:

```bash
ai-metadata-cleaner delete ./images --output ./cleaned-images --preserve-timestamps
```
...
stops. Use `--workspace ./ai-metadata-cleaner-workspace` to keep uploaded originals
...
```bash
ai-metadata-cleaner delete ./images --json-summary --quiet
```

Preview work without creating files:

```bash
ai-metadata-cleaner delete ./images --dry-run
```
...
```bash
ai-metadata-cleaner --verbose view sample.jpg
```
